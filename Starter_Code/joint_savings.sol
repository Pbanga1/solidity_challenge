...
contract JointSavings {
    ...
    - Two variables of type `uint256 public` named `lastWithdrawAmount` and `contractBalance`.
    */
    ...
    /*
    Defining a function named **withdraw** that will accept two arguments.
    ...
    */
    function withdraw(uint amount, address payable recipient) public {
        ...
        /*
        Defining a `require` statement that checks if the `recipient` is equal to either `accountOne` or `accountTwo`. The `requiere` statement returns the text `"You don't own this account!"` if it does not.
        */
        
        require(recipient == accountOne || recipient == accountTwo, "You don't own this account!");

        /*
        Defining a `require` statement that checks if the `balance` is sufficient to accomplish the withdraw operation. If there are insufficient funds, the text `Insufficient funds!` is returned.
        */
    ...
    */
    ...
    /*
    Finally, adding the **default fallback function** so that your contract can store Ether sent from outside the deposit function.
    */
